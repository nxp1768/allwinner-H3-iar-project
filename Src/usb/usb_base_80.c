...
int ep0_recv_op(void);

extern int fastboot_data_flag;
volatile int sunxi_usb_burn_from_boot_init = 0;

volatile int IsSent = 0;
/*
 ************************************************************************************************************
 *
 *                                             function
...
int endpont_cont = 0; //记录端口0还有数据需要分包发送
int ep0_int = 0;
...
void sunxi_usb_irq(void *data)
{
	...
	if (misc_irq & USBC_INTUSB_RESET)
	{
		...
		usb_dma_set_pktlen(sunxi_udc_source.dma_send_channal,
		HIGH_SPEED_EP_MAX_PACKET_SIZE);
		usb_dma_set_pktlen(sunxi_udc_source.dma_recv_channal,
		HIGH_SPEED_EP_MAX_PACKET_SIZE);
	...
	if (misc_irq & USBC_INTUSB_SUSPEND)
	{
		...
		/* clear interrupt */
		USBC_INT_ClearMiscPending(sunxi_udc_source.usbc_hd,
		USBC_INTUSB_SUSPEND);
	}
	/* DISCONNECT */
	...
	if (rx_irq & (1 << SUNXI_USB_BULK_OUT_EP_INDEX))
	{
		...
		/* Clear the interrupt bit by setting it to 1 */
		USBC_INT_ClearEpPending(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX,
...
int sunxi_usb_init(int delaytime)
{
	...
	USBC_INT_EnableUsbMiscUint(sunxi_udc_source.usbc_hd, USBC_INTUSB_SUSPEND | USBC_INTUSB_RESUME | USBC_INTUSB_RESET | USBC_INTUSB_SOF);

	/* enbale ep0_tx_irq */
	USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, SUNXI_USB_CTRL_EP_INDEX);
	USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX, SUNXI_USB_CTRL_EP_INDEX);  //add by wg
	...
	__sunxi_usb_init_fail: if (sunxi_udc_source.dma_send_channal)
	{
...
static void __usb_send_by_dma_isr(void *p_arg)
{
	...
	old_ep_idx = USBC_GetActiveEp(sunxi_udc_source.usbc_hd);
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_IN_EP_INDEX);			//选择RXEP
	...
	if (usb_dma_trans_unaliged_bytes)
	{
		...
		this_len = USBC_ReadLenFromFifo(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX);
		fifo = USBC_SelectFIFO(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_IN_EP_INDEX);
		USBC_WritePacket(sunxi_udc_source.usbc_hd, fifo, this_len, usb_dma_trans_unaligned_buf);
		__usb_writecomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, 1);		//返回状态
		usb_dma_trans_unaliged_bytes = 0;
	}
	//如果当前dma传输的不是完整包，则需要手动清除中断
	if (sunxi_ubuf.request_size % sunxi_udc_source.bulk_ep_max)
	{
		USBC_Dev_ReadDataStatus(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, 1);
...
//#define  devic_ctl_reg   (*(volatile unsigned short *)(0x01c19082))
__no_init volatile unsigned short devic_ctl_reg@0x01c19082;
unsigned short ii = 0;
...
static void __usb_writecomplete(__hdle hUSB, u32 ep_type, u32 complete)
{
	...
	/*
	 unsigned short ii = devic_ctl_reg;
	 //ii = devic_ctl_reg;
	 if(devic_ctl_reg)
	 {
	 ;
...
void __usb_writecomplete_wg(__hdle hUSB, u32 ep_type, u32 complete)		//add by wg for setup > 64
{
	int cont = 0;
	USBC_Dev_WriteDataStatus(hUSB, ep_type, complete);
...
static void __usb_interuput_ep_reset(void)
{
	u8 old_ep_index = 0;
	...
	/* tx */
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_IN_EP_INDEX);
	USBC_Dev_ConfigEp(sunxi_udc_source.usbc_hd, USBC_TS_TYPE_INT, USBC_EP_TYPE_TX, 1, sunxi_udc_source.bulk_ep_max & 0x7ff);
	USBC_ConfigFifo(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, 1, sunxi_udc_source.fifo_size, (uint) sunxi_udc_source.bulk_out_addr);
	USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, SUNXI_USB_BULK_IN_EP_INDEX);
	/* rx */
	//USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_OUT_EP_INDEX);
	//USBC_Dev_ConfigEp(sunxi_udc_source.usbc_hd, USBC_TS_TYPE_INT, USBC_EP_TYPE_RX, 1, sunxi_udc_source.bulk_ep_max & 0x7ff);
	//USBC_ConfigFifo(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX, 1, sunxi_udc_source.fifo_size, (uint)sunxi_udc_source.bulk_in_addr);
	//USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX, SUNXI_USB_BULK_OUT_EP_INDEX);
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, old_ep_index);
...
static int __usb_write_fifo_setup(void *buffer, unsigned int buffer_size)		//add by wg
{
	u32 old_ep_idx = 0;
	u32 fifo = 0;
	u32 transfered = 0, temp = 0;
	u32 left = 0;
	u32 this_len;
	...
	{
		//this_len = MIN(sunxi_udc_source.fifo_size, left);
		this_len = MIN(64, left);
		temp = (u32) buffer + transfered;
		fifo = USBC_SelectFIFO(sunxi_udc_source.usbc_hd, SUNXI_USB_CTRL_EP_INDEX);
		this_len = USBC_WritePacket(sunxi_udc_source.usbc_hd, fifo, this_len, (void *) temp);
		...
		{
			endpont_cont = 1;
			__usb_writecomplete_wg(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0, 0);
			while (endpont_cont)
				;
		}
		else
		{
			__usb_writecomplete_wg(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0, 1);
		}
...
int sunxi_udc_start_sent_by_dma(uint mem_buf, uint length)
{
	uint old_ep_idx;
 ...
 ************************************************************************************************************
 */
void sunxi_udc_send_setupXXXXX(uint bLength, void *buffer)
{
	u32 fifo = 0;
...
void TX_task(void)
{
	...
	sunxi_usb_dbg("DataRemain:%d  cont:%d \n", DataRemain, cont);

	fifo = USBC_SelectFIFO(sunxi_udc_source.usbc_hd, SUNXI_USB_CTRL_EP_INDEX);
	USBC_WritePacket(sunxi_udc_source.usbc_hd, fifo, cont, (void *) pData);

	if (cont == 64) //需要单独发送空包
	{
		USBC_Dev_WriteDataStatus(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0, 0);
		//__usb_writecomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0, 0);
	}
	else
	{
		ep_status = USB_STATE_IDLE;

		USBC_Dev_WriteDataStatus(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0, 1);
		//__usb_writecomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0, 1);
		// USBC_Dev_IsWriteDataReady(sunxi_udc_source.usbc_hd, ep_type));
...
void sunxi_udc_send_setup(uint bLength, void *buffer)
{
	...
	{
		__usb_write_fifo_setup(buffer, bLength);
		/*
		 ep0_sentPtr = (char *)buffer;
		 DataRemain =  bLength;
		 ulOUTDataSize = bLength;
		 ep_status = USB_STATE_TX;
		 TX_task();  //发送数据
		 */
	}
	else
...
{
	//sunxi_udc_send_setup(0,"");//add by wg
	return 0;
}
/*
 ************************************************************************************************************
...
int sunxi_udc_send_data(void *buffer, unsigned int buffer_size)
{
	...
	__usb_write_fifo((uchar *) buffer, buffer_size);
...
enum ep0_state
{
	EP0_IDLE, EP0_IN_DATA_PHASE, EP0_OUT_DATA_PHASE, EP0_END_XFER, EP0_STALL,
};
int ep0state = EP0_IDLE;
...
int ep0_recv_op(void)
{
	...
	{
		printf("handle_ep0: ep0 setup end\n");
		USBC_Dev_Ctrl_ClearSetupEnd(sunxi_udc_source.usbc_hd);
		ep0state = EP0_IDLE;
	}
	...
	case EP0_IN_DATA_PHASE: /* GET_DESCRIPTOR etc */
		if (USBC_Dev_IsWriteDataReady(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0))
		{
	...
	if (USBC_Dev_IsReadDataReady(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0))
	{
		...
		if (!ep0_stage)
		{
			...
			sunxi_usb_dbg("->%02x %02x %04x %04x %04x\n", sunxi_udc_source.standard_reg.bmRequestType, sunxi_udc_source.standard_reg.bRequest, sunxi_udc_source.standard_reg.wValue,
					sunxi_udc_source.standard_reg.wIndex, sunxi_udc_source.standard_reg.wLength);
		}
		else
		{
			status = __usb_read_ep0_data(sunxi_usb_ep0_buffer, ep0_stage);
			printf("sunxi_usb_ep0_buffer \n");
		}
	...
	if (USB_REQ_TYPE_STANDARD == (sunxi_udc_source.standard_reg.bmRequestType & USB_REQ_TYPE_MASK))
	{
		...
		case USB_REQ_GET_DESCRIPTOR:		//   0x06
		{
			...
			if (USB_DIR_IN == (sunxi_udc_source.standard_reg.bmRequestType & USB_REQ_DIRECTION_MASK))
			{
				...
				else if (sunxi_udc_source.standard_reg.bmRequestType == 0x81)		//add by wg
				{
					ret = sunxi_udev_active->standard_req_op(USB_REQ_GET_DESCRIPTOR, &sunxi_udc_source.standard_reg, sunxi_usb_ep0_buffer);		//wg
				}
			}
	...
	__ep0_recv_op_err: USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, old_ep_index);
