...
int spic_init(uint spi_no)
{
      ...
      spi_tx_dma_hd = sunxi_dma_request(DMAC_CFG_SRC_ADDR_TYPE_LINEAR_MODE);
      spi_rx_dma_hd = sunxi_dma_request(DMAC_CFG_SRC_ADDR_TYPE_LINEAR_MODE);
      ...
      spi_rx_dma.cfg.src_drq_type     = DMAC_CFG_SRC_TYPE_SPI0;  //SPI0
     ...
     // sunxi_dma_setting(spi_tx_dma_hd, (void *)&spi_tx_dma);
      ...
      //--------read version reg---
       writel(1<<31, SPI_GC_REG);

      reg_val = readl(SPI_GC_REG);
      reg_val |= SPI_GC_EN;
      writel(reg_val, SPI_GC_REG);

      version =  readl(SPI_VER_REG);
      printf("SPI Version:%08X",version);


      spi_clk = spi_set_src_clk(0, 2, SPI_MCLK);

      printf("spi set:%dMHz  return:%dMHz \n",SPI_MCLK,spi_clk);


      reg_val =  ((0<<7)|(1<<1)|1);

      SPI_CTL_reg[0] =  reg_val;

      writel(reg_val, SPI_CTL);
