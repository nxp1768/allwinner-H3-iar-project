...
typedef struct sunxi_dma_channal_set_t
{
	...
	volatile unsigned int parameters;		//参数
        volatile unsigned int reserv[3];
        volatile unsigned int FDESC_ADDR;
         volatile unsigned int PKG_NUM_REG;
}
...
************************************************************************************************************
*/
static sunxi_dma_start_t dma_mac[SUNXI_DMA_MAX];//
...
void sunxi_dma_init(void)
{
	...
	for(i=0;i<SUNXI_DMA_MAX;i++)
	{
		...
		//dma_channal_source[i].config  = (sunxi_dma_start_t *)malloc_noncache(sizeof(sunxi_dma_start_t));
                dma_channal_source[i].config  = (sunxi_dma_start_t *)&dma_mac[i];
	}
...
**********************************************************************************************************************
*/
sunxi_dma_int_set   *dma_int_S 	 = (sunxi_dma_int_set *)SUNXI_DMA_BASE;
...
int sunxi_dma_querystatus(uint hdma)
{
	...
	return (dma_int_S->status >> channal_count) & 0x01;
}
/*
************************************************************************************************************
*
