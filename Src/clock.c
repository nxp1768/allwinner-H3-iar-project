...
unsigned int *cpu0_ctl = (unsigned int *)(0x1f01c44);
unsigned int *cpu1_ctl = (unsigned int *)(0x1f01c84);
unsigned int *cpu2_ctl = (unsigned int *)(0x1f01cc4);
unsigned int *cpu3_ctl = (unsigned int *)(0x1f01d04);
...
int sunxi_clock_get_pll5(void)
{
	unsigned int reg_val;
	int factor_n, factor_k, pll5;

	reg_val = readl(CCM_PLL5_DDR_CTRL);
	factor_n = ((reg_val >> 8) & 0x1f) + 1;
	factor_k = ((reg_val >> 4) & 0x03) + 1;
	pll5 = 24 * factor_n * factor_k/2;
	return pll5;
}
...
{
	struct sunxi_ccm_reg_l * const ccm =
		(struct sunxi_ccm_reg_l *)SUNXI_CCM_BASE;
	const int p = 0;
	int k = 1;
	int m = 1;
